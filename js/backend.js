$(document).ready(function() {
    ...
    $('.save-fullname').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(html){
                ...
                //                console.log(html);
                if (html == 'true') {
                    $(".profile-left").html('<div class="alert alert-success"><strong>Message Sent!</strong></div>');
    ...
    $('.save-basic-info').on('click', function() {
        ...
        $.ajax({
            type:"POST",
            url:"editProfile.php",
            data:"id=" + id + "&ssn=" + ssn + "&bdate=" + bdate + "&loc=" + loc + "&type=basic-info",
            success: function(html){
                //                console.log(html);
                if (html == 'true') {
                    $(".profile-left").html('<div class="alert alert-success"><strong>Message Sent!</strong></div>');
    ...
    $('.save-contact-info').on('click', function() {
        ...
        $.ajax({
            type:"POST",
            url:"editProfile.php",
            data:"id=" + id + "&email=" + email + "&phone=" + phone + "&type=contact-info",
            success: function(html){
                console.log(html);
                if (html == 'true') {
                    $(".profile-right-up").html('<div class="alert alert-success"><strong>Message Sent!</strong></div>');
    ...
    $('.save-company-info').on('click', function() {
        ...
        $.ajax({
            type:"POST",
            url:"editProfile.php",
            data:"id=" + id + "&uname=" + uname + "&pass=" + pass + "&type=company-info",
            success: function(html){

                console.log(html);
                if (html == 'true') {
                    $(".profile-right-down").html('<div class="alert alert-success"><strong>Message Sent!</strong></div>');
...
$(document).on('click','.sal',function(event)
	...
	$(document).on('focusout','.sal',function(event)
	{
	   ...
	   var c=$("#Display").find('tr:eq('+ rowIndex + ')').find('td:eq(2)');
	   event.preventDefault();
	   var test = $(this).html();
   ...
   function search_table(value){ 
	    var selected = $('#choice').children("option:selected").val();
	    var selection;
	    if(selected == "email")  
	        selection = 5;
	    if(selected == "ssn")
		...
		$('#Display tr').each(function(){
			 ...
			 var x = $(this).find('td:eq('+ selection +')');
				  if(x.text().toLowerCase().indexOf(value.toLowerCase()) >= 0)  
				  { 
					found = 'true';  
				  }  
